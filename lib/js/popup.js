class Popup{
  constructor() {
    this.register = {
      ...
      update(ID, data) {
        if(ID && data && data instanceof Object) {
          if(this.exist(ID)) {
            Object.assign(this.windows[ID], data);
            console.info("Window instance was updated");
          } else console.error("No such ID in popup cache");
        } else console.warn("Can't update popup instance, no data provided");
      },
      clean(wID) {
        if(wID) {
          if(this.exist(wID)) {
   ...
   * - LEAVE_ii  => When unmount called, do not remove nodes from translation list 
   * - JUST_DOM => When unmount called, unmount just in DOM, leave data in register
   * - EXIST => Just render in to DOM
   */
  mount(vNode, closeTrigger, style, func, parms, flag) {
    ...
    try {
      ...
      if(Core.arrHas(flag, "EXIST")) {
        windowNode.setAttribute("id", stateParams.oldID);
      } else windowNode.setAttribute("id", wID);
      ...
      if(Core.arrHas(flag, "NO_STORE")) {
        console.info("Window instance wasn't stored in cache");
      } else if(Core.arrHas(flag, "EXIST")) {
          this.register.update(stateParams.oldID, {
            vNode: windowNode,
          });
      } else {
      ...
      if(!Core.arrHas(flag, ["NO_CLOSE_EVENT", "EXIST"], "ONE")) {

        closeTrigger.addEventListener("click", () => { 
          this.unmount(wID, flag); 
        });
   ...
   * - LEAVE_ii  => Do not remove nodes from translation list 
   * 
   */
