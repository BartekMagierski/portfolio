      ...
      navs: {},
			...
			set DOMref(ref) {
				if(ref instanceof Object) {
					for(let[key, VNode] of Object.entries(ref)) {
						if(!Reflect.has(this.DOMrefs, key)) {
							Object.defineProperty(this.DOMrefs, key, {
								value: VNode,
								writable: false
							});
						} else throw `DOM reference: ${nodeID} already exist`
					}
				}
			},
			getDOMref(refID) {
				if(Reflect.has(this.DOMrefs, refID)) {
					return this.DOMrefs[refID];
				} else {
						console.warn(`DOM reference: ${refID} was requested, but it wasn't defined`);
						return null;
				}
			}
    };	
