class Language {
	 ...
	 * RETURN_NOW
	 */
	async prepare(data, promise, flag, info) {

		if(!Array.isArray(flag)) flag = [flag];
    let worker = await workers.requestSlot("LangBlobURL");
		let init = functionBody.bind(this); 

		// If there is no promise then make one, because this is async function
		if(!promise) {
			if(!(Core.arrHas(flag, "RETURN_NOW"))) {
				return new Promise(async (success, failure) => {
					await init(success, failure);
				});
			} else return init();
			
		} else init(promise[1], promise[0]);

    function functionBody(success, failure) {
      ...
      worker.onmessage = async (response) => { 
        ...
        this.packages.list[id] = JSON.parse( await response.data.text() )
				...
				if(info) console.info(`Package "${info}" was prepared as ${id}`);
      ...
      worker.onerror = function(response) { 
        SC.workers.releseSlot(worker);
			...
			if(Core.arrHas(flag, "RETURN_NOW")) return id;
	 ...
	 * @param {Array} promise 0=> success 1=> failure
	 */
	async translate(mode, pkgId, param, promise){
		...
		try {
			...
			return response;
				...
				console.error(e);
				console.groupEnd();
				if(promise) promise[1](false);
				return mode === "text" ? "undefined" : undefined;
		}
		...
		function w() {
			return new Promise((res,rej) => {
				Core.mutationOccurs(this.packages.ready, pkgId, 50, 5000, [res,rej]);
			});
		}
		/** Translate text */
		function t(strId) {
