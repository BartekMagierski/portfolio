...
class Core {
  ...
  /**
   * Watch for property(indicator) in object/array (1 dimm)
   * @param {*} haystack Object or Array 
   * @param {*} needles String or Array of strings
   * @param {Number} interval 
   * @param {Number} timeOut 
   * @returns Promisse
   */
	static watch(haystack, needles, interval, timeOut) {

    if(!haystack || !haystack instanceof Object && !Array.isArray(haystack)) {
      console.error("Missing or wrong instance of haystack for watch function");
      console.trace();
      return undefined;
    }
    if(!needles) {
      console.warn("Missing needless for watch function");
      console.trace();
      return true;
    }
    if(!Array.isArray(needles)) needles = [needles];
    if(needles.some(function(x){ return typeof x !== "string"; })) {
      console.groupCollapsed("Watch function exception");
      console.error("Needle have to be a string");
      console.log("needle: ", needles);
      console.trace();
      console.groupEnd();
      return undefined;
    }
    ...
    if(!interval || typeof interval !== "number") {
      interval = 50;
      console.info(`Using default interval(${interval}) in watch function`);
    }
    if(!timeOut  || typeof timeOut !== "number") {
      timeOut = 10000;
      console.info(`Using default timeOut(${timeOut}) in watch function`);
    }
    ...
    return new Promise(function(resolve, reject) {
      ...
      let observationMach = new Array(needles.length);

      observation = setInterval(() => {

        needles.forEach(function(needle, index) {
          observationMach[index] = haystack instanceof Object
            ? needle in haystack
            : haystack.includes(needle);
        });

        if(observationMach.every(function(x){ return x === true; })) {
  ...
  */
  ...
  static loadAccounts() {
    ...
    Core.ajax("LoadAccounts",{
      address: MainPhp,
      options: {
        method: "GET"
      },
      values: {
        callCore: "loadAccounts"
      }
    }).then(function(raw) {
        
        if(raw.status === "success") {

          let blob = new Blob([raw.data], {type: 'text/javascript'});
        ...
        } else { console.error(`Accounts module was not loaded, reason: ${raw.message}`); }
